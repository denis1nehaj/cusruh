...
class photos : Fragment() {
    ...
    override fun onCreateView(
        ...
        binding.photosToCalculator1.setOnClickListener {
            val preferences = activity?.getSharedPreferences("pref", Context.MODE_PRIVATE)
            with(preferences?.edit()) {
                this!!.putString("FurnitureType", "Стол с Тумбой")
                apply()
            }
            root.findNavController().navigate(R.id.action_navigation_photos_to_navigation_profile)
        }
        return root
